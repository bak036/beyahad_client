...
const ShortCodeComponent : React.FC<Props> = ({
	onShortCodeChanged,
	...
	const [shortCodeErrorVar, setShortCodeErrorVar] = useState<string>('');
	const [numOfPayment, setNumOfPayment] = useState<string>('');
	const [numOfPaymentErrorVar, setNumOfPaymentErrorVar] = useState<string>('');
	const [checkboxRef, setCheckboxRef] = useState<any>(React.createRef());
	const [prevShortCodeError, setPrevShortCodeError] = useState<string>('');
	...
	useEffect(() => {
		if(prevShortCodeError != shortCodeError){
			setPrevShortCodeError(shortCodeError)
			setShortCodeErrorVar(shortCodeError)
		}
	},[shortCodeError])

	const onShortCodeChangedFunc = (value) => {
		const shortCode = value;
		onShortCodeChanged(value);
		clearError();
	};
	const clearError = () => {
		if (ValidationService.validateShortCode(creditCardStore.shortCode)) 
			setShortCodeErrorVar('')
		if (agreement)
			setAgreementError('')
	...
	const validate = () => {
		let retVal = true;
		if (!ValidationService.validateShortCode(creditCardStore.shortCode)) {
			setShortCodeErrorVar('יש להזין קוד מקוצר בעל 5 ספרות בלבד')
			retVal = false;
		}
		...
		if (!disableNumOfPayments) {
			if (numOfPayment.length === 0) {
				setNumOfPaymentErrorVar('יש להזין מספר תשלומים')
				retVal = false;
			}
		}
		if (addedValidations) {
	...
	return (
		<div className='payment-form-with-short-code'>
...
export default observer(ShortCodeComponent)
