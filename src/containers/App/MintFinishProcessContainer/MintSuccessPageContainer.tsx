...
const MintSuccessPageContainer : React.FC<Props> = ({
}) => {
	...
	useEffect(() => {
		...
		return (
			<div className='mint-error-container'>
				<div className='message'>{<CustomHeader text={Lang.format('MintSuccessPage')} />}</div>
				<div className='message'>{<CustomHeader text={Lang.format('MintText')} />}</div>
			</div>
		);
	}
export default observer(MintSuccessPageContainer)
